...
import TermsAndConditionsScreen from "./screens/TermsAndConditionsScreen"; // ✅ Add this import
import {
  NavigationProvider,
  TaskRemovedBehavior,
...
export default function App() {
  ...
  // Determine which screen to show based on auth state and flags
  const shouldShowSuccess = user && showSuccessScreen && !isAdmin;
          ...
          <NavigationContainer>
            ...
            >
              {!user ? (
                ...
                <>
                  {isAdmin ? (
                    // Admin users see only Admin screen
                    <Stack.Screen 
                      name="Admin" 
                      component={AdminScreen}
                    />
                  ) : (
                    // Regular users see these screens
                    <>
                      <Stack.Screen 
                  ...
                  <Stack.Screen
                    ...
                    options={({ navigation }) => ({
                      headerShown: true,
                      headerTitle: () => (
                        <Text style={styles.TemplesText}>Temples Nearby</Text>
                      ),
                      ...
                      headerRight: () => (
                        <TouchableOpacity
                          ...
                          onPress={async () => {
                            try {
                              setShowSuccessScreen(false); // Reset flag on logout
                              await auth().signOut();
                            } catch (error) {
